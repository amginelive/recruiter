...
class CompanyViewTests(BaseTest):
    ...
    def test_company_detail_page_company_owner(self):
        ...
        user_note = G(
            ...
            note_to=self.company.owner.user
        )

        connection = G(
            Connection,
            connectee=self.user_candidate,
            connecter=self.company.owner.user
        )
        ...
        message_agent = G(
            Message,
            author=self.company.owner.user,
            conversation=conversation
        )
        message_candidate = G(
            Message,
        ...
        self.assertEqual(response.context.get('current_agent'), self.company.owner)

        self.assertEqual(response.context.get('user_note'), UserNote)
        self.assertIn(user_note, response.context.get('user_notes'))

        self.assertEqual(response.context.get('current_agent_connection'), connection)
        self.assertEqual(response.context.get('connection_request'), ConnectionRequest)
        self.assertEqual(response.context.get('current_agent_connection_request'), connection_request)
        ...
        self.assertEqual(response.context.get('last_person_in_contact').user, self.company.owner.user)
        self.assertEqual(response.context.get('last_person_added_manual_track'), self.company.owner.user)
    ...
    def test_company_detail_page_company_staff(self):
        ...
        self.assertEqual(response.context.get('company'), self.company)
        self.assertIn(company_request_invitation, response.context.get('invitation_requests'))
        self.assertEqual(response.context.get('current_agent'), new_agent)
